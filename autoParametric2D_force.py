...
result_dir, result_name = getInputs(
    fields=(("Vi tri luu ket qua:", "E:\NCKH\soft_robot\Optimisation\\"), ("Ten file:",
                                              "force_output")),
...
for n in range(instanceNum):
    ...
    BaseSketchC = mdb.models[str('Model-{}'.format(n + 1))].ConstrainedSketch(
        name='__profile__', sheetSize=200.0)
    BaseSketchC.rectangle(point1=(0.0, 0.0),
                          point2=(10, 5))
    ...
    a.InstanceFromBooleanMerge(name='Merged-Body',
                               instances=(a.instances['Base-A'],
                                a.instances['Base-B'],
                                a.instances['Paper'],
                                a.instances['Main-Body']),
    ...
    a4.ReferencePoint(point=a4.instances['Base-C'].InterestingPoint(edge=e1[0], 
        rule=MIDDLE))
    ...
    mdb.models[str('Model-{}'.format(n + 1))].Coupling(name='Coupling', controlPoint=region1, 
    ...
    mdb.models[str('Model-{}'.format(n + 1))].RigidBody(name='Rigid', refPointRegion=region1, 
        bodyRegion=region2)
    region = a1.instances['Base-C'].sets['Set-Base-C']
    mdb.models[str('Model-{}'.format(n + 1))].EncastreBC(name='Fix_Base', createStepName='Initial', 
    ...
    mdb.models[str('Model-{}'.format(n + 1))].interactionProperties['IntProp-1'].TangentialBehavior(
    ...
    a1 = mdb.models[str('Model-{}'.format(n + 1))].rootAssembly
    region2=a1.instances['Base-C'].surfaces['Top-Suf-BaseC']
    mdb.models[str('Model-{}'.format(n + 1))].SurfaceToSurfaceContactStd(name='Int-1', 
