    ...
    for chamber_num in range(1,int(maxChamberNum)+1):
        ...
        if(chamberSpaceOptimizeTimes > 1):
            for i in range(1,chamberSpaceOptimizeTimes):
                listChamberSpaceValues.append(minChamberSpace + (maxChamberSpace-minChamberSpace)/(chamberSpaceOptimizeTimes-1) * i)
        ...
        for chamber_space in listChamberSpaceValues:
            ...
            if(chamberWallThicknessOptimizeTimes > 1):
                for i in range(1,chamberWallThicknessOptimizeTimes):
                    listChamberWallThicknessValues.append(minChamberWallThickness + (maxChamberWallThickness-minChamberWallThickness)/(chamberWallThicknessOptimizeTimes-1) * i)
            ...
            for chamber_wallThickness in listChamberWallThicknessValues:
                ...
                subprocess.run(
                    ["abaqus", "cae", "noGUI=./abaqusScript/autoParametric2DnoGUI.py"], shell=True,stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                print("*********************")
